...
if command -v conda &>/dev/null; then
  # Check if the 'user' environment exists
  if ! conda env list | grep -qE "^user\s"; then
    echo "[conda] Environment 'user' not found. Attempting to create..."

    # Attempt to create the environment
    if conda create -n user python=3.11 pip -y; then
      echo "[conda] 'user' environment created successfully."
      conda activate user
    else
      # Failure case
      echo "----------------------------------------------------"
      echo "ERROR: Failed to create conda environment 'user'."
      echo "Please check your internet connection or conda permissions."
      echo "----------------------------------------------------"
    fi
    sleep 2
  else
