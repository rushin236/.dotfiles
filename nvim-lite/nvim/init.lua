...
vim.opt.shortmess:append("c") -- Hides "match x of y" messages
...
vim.api.nvim_create_user_command("LspDiagnosticsBuffer", function()
	...
	for _, d in ipairs(diags) do
		local severity_name = vim.diagnostic.severity[d.severity] or "INFO"
		...
		max_line_len = math.max(max_line_len, #msg)
	end

	-- Limit how wide / tall the window can be
	...
	local max_width = math.min(
		max_line_len,
		80, -- hard cap
		math.floor(columns * 0.6) -- 60% of screen
	)

	local max_height = math.min(
		#lines,
		math.floor(lines_screen * 0.5) -- at most 50% of screen height
	)
	...
	local _, win = vim.lsp.util.open_floating_preview(lines, "plaintext", float_opts)

	-- enable wrapping so long lines don’t force a super wide window
...
local function FloatingTerminal()
	...
	if not has_terminal then
		...
		-- Modern replacement for termopen(): terminal job in current buffer
...
-- Key mappings
vim.keymap.set("n", "<c-\\>", FloatingTerminal, {
	noremap = true,
	silent = true,
...
-- Formatting
-- ===============================================================================
local function safe_restore_cursor(cursor)
	local bufnr = vim.api.nvim_get_current_buf()
	local line_count = vim.api.nvim_buf_line_count(bufnr)

	-- Clamp row to [1, line_count]
	local row = cursor[1]
	if row < 1 then
		row = 1
	...
	-- Get the line length at the new row
	local line = vim.api.nvim_buf_get_lines(bufnr, row - 1, row, false)[1] or ""
	local max_col = #line

	-- Clamp col to [0, max_col]
	local col = cursor[2]
	if col < 0 then
		col = 0
...
local function run_formatter(cmd, args, name)
	args = args or {}
	name = name or cmd
	...
	if vim.system then
		...
		result = vim.system(argv, { text = true, stdin = input }):wait()
	else
		-- Older Neovim fallback
		local full = cmd .. " " .. table.concat(args, " ")
		local out = vim.fn.system(full, input)
		result = {
			code = vim.v.shell_error,
			stdout = out,
			stderr = "", -- can't separate here
		}
	end

	-- On error: keep buffer & cursor, just show message
	if result.code ~= 0 then
		local msg = result.stderr
		if msg == nil or msg == "" then
			msg = string.format("%s failed (exit %d)", name, result.code)
		end
		vim.notify(msg, vim.log.levels.ERROR)
		return
	end

	-- On success: replace buffer
...
local function format_buffer()
	...
	if ft == "python" then
		if vim.fn.executable("ruff") == 1 then
			run_formatter("ruff", { "format", "-" }, "ruff format")
		else
...
vim.api.nvim_create_autocmd("LspAttach", {
	callback = function(ev)
		...
		-- 🔁 unified formatting: use format_buffer() with blink-style key
		bufmap({ "n", "v" }, "<leader>cf", format_buffer)
...
-- Force completion (like blink's <C-Space>)
vim.keymap.set("i", "<C-Space>", function()
	-- First try path completion if we're in a path context
	if is_path_context() then
		trigger_path_completion()
	else
		trigger_lsp_completion()
	end
end, { silent = true })

-- Close menu (and otherwise behave like normal <C-e>)
vim.keymap.set("i", "<C-e>", function()
	if vim.fn.pumvisible() == 1 then
		-- <C-e> in insert mode already cancels completion, just send it
		return "<C-e>"
	end
	return "<C-e>"
end, { expr = true, silent = true })

-- Signature help (blink uses <C-k> to toggle)
vim.keymap.set("i", "<C-k>", function()
	-- If you want true "toggle", you could track a flag; for now just show
